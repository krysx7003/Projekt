...
def createPlot(trou_data,hake_data,herr_data):
    ...
    plt.plot(x,trou_data['Wartosc'],color='r', linestyle='-', linewidth=2)
    plt.plot(x,hake_data['Wartosc'],color='b', linestyle='--', linewidth=1.5)
    plt.plot(x,herr_data['Wartosc'],color='g', linestyle=':', linewidth=3)
    plt.xticks(x,trou_data['Rok'])
    plt.legend(["Pstrąg","Morszczuk","Śledź"], loc="center right")
    plt.xlabel("Lata")
    plt.ylabel("Cena[zł]")
    plt.grid(True)
    plt.savefig("task5.webp", format="webp")
    plt.show()
